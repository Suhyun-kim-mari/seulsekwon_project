...
def inject_custom_css():
    ...
    <style>
        ...
        /* 랜딩 페이지 전용 스타일 */
        .hero-section {{
            background: linear-gradient(135deg, {THEME['error']} 0%, #dc2626 100%);
        ...
        .landing-card:hover {{
            ...
            box-shadow: 0 25px 35px -5px rgba(59, 130, 246, 0.15);
    ...
    <a href="https://forms.gle/p7WY26UqJzxfPsNg8" target="_blank" class="error-report-button">
...
def render_landing_page():
    ...
    <div class="hero-section">
        <div style="font-size: 1.2rem; font-weight: 800; color: rgba(255, 255, 255, 0.7); letter-spacing: 0.2em; margin-bottom: 1rem; text-transform: uppercase;">
            Lifestyle Intelligence
        </div>
        <div class="hero-title">🏙️ 서울 슬세권 분석 서비스</div>
        <p class="hero-subtitle">데이터로 증명하는 내 집 앞 생활 인프라의 가치</p>
    </div>
    ''', unsafe_allow_html=True)

    # 2. Hero Search Bar
    with st.container():
        st.markdown('<div style="max-width: 900px; margin: -5rem auto 5rem auto; position: relative; z-index: 10;">', unsafe_allow_html=True)
        with st.form("landing_search"):
            st.markdown(f'<p style="font-weight:800; font-size:1.3rem; margin-bottom:1.5rem; text-align:center; color: {THEME['secondary']}; letter-spacing:-0.03em;">분석하고 싶은 주소나 랜드마크를 입력하세요</p>', unsafe_allow_html=True)
            col1, col2 = st.columns([4, 1.2])
            with col1:
                query = st.text_input("📍 위치 검색", placeholder="예: 강남역, 성수동 아크로포레스트, 서울시청 등", label_visibility="collapsed")
            with col2:
                btn_start = st.form_submit_button("지금 분석하기 🚀", use_container_width=True)
            
            if btn_start and query:
                with st.spinner("위치를 검색하고 있습니다..."):
                    ...
                    if res:
                        ...
                        st.error("해당 위치를 찾을 수 없습니다. 정확한 명칭이나 주소를 입력해 주세요.")
    ...
    with col1:
        ...
        <div class="landing-card">
            ...
            <div class="card-title">팀 소개: 데이터를 9해조</div>
            <div class="card-text">
                저희 팀은 서울 시민들이 살기 좋은 환경을 데이터로 쉽게 파악할 수 있도록 <b>공공 데이터 분석 도구</b>를 개발했습니다. 
                복잡한 통계를 직관적인 지표로 변환하여 주거 선택의 기준을 제시합니다.
            </div>
        </div>
        ''', unsafe_allow_html=True)
    ...
    with col2:
        ...
        <div class="landing-card">
            ...
            <div class="card-title">'슬세권'이란?</div>
            <div class="card-text">
                '슬리퍼'와 '역세권'의 합성어로, <b>편한 복장으로 도보 10분 내</b>에 카페, 마트, 병원, 은행 등 필수 인프라를 누릴 수 있는 주거 권역을 말합니다. 
                주변 인프라를 재미 있으면서 쉽게 이해할 수 있는 현대 주거 트렌드 지표입니다.
            </div>
        </div>
        ''', unsafe_allow_html=True)
    ...
    with col3:
        ...
        <div class="landing-card">
            <span class="stat-icon">📈</span>
            <div class="card-title">점수 계산의 원리</div>
            <div class="card-text">
                서울시 내 10만 개 이상의 인프라 데이터를 분석하여 <b>7대 핵심 카테고리</b>(생활, 교통, 의료, 안전, 교육, 자연, 금융)에 대해 가중치를 적용합니다. 
                단순 개수가 아닌 거리와 편의성 비중을 고려한 복합 지수입니다.
            </div>
        </div>
        ''', unsafe_allow_html=True)
...
def render_dashboard():
    ...
    with st.container():
        # [Box 1] 제목 박스
        st.markdown(f'''
        <div class="dashboard-card" style="text-align: center; margin-bottom: 2rem; position: relative;">
    ...
    with tab1:
        ...
        with c2:
            ...
            st.markdown('</div>', unsafe_allow_html=True)
        with c3:
            st.markdown('<div class="dashboard-card" style="height: 100%;"><h4>📋 주요 시설 통계</h4>', unsafe_allow_html=True)
            stats_df = pd.DataFrame(counts.items(), columns=['분류', '개수']).sort_values('개수', ascending=False)
            st.dataframe(stats_df, hide_index=True, use_container_width=True, height=380)
            st.markdown('</div>', unsafe_allow_html=True)
    ...
    with tab2:
        ...
        with st.spinner("주변 실거래 데이터 분석 중..."):
            recent_re = filter_data_within_radius(st.session_state.config['coords'][0], st.session_state.config['coords'][1], st.session_state.re_data, 3.0)
        ...
        if not recent_re.empty:
            ...
            with col1:
                ...
                fig_scatter = px.scatter(recent_re, x="ARCH_AREA", y="price_억", color="price_억", color_continuous_scale="Viridis", hover_data=["BLDG_NM", "RCPT_YR"], labels={'ARCH_AREA': '전용면적 (㎡)', 'price_억': '거래가 (억 원)'})
                fig_scatter.update_layout(paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', font=dict(family="Pretendard, -apple-system, BlinkMacSystemFont, system-ui, sans-serif", color=THEME['secondary']), margin=dict(t=10, b=10, l=10, r=10), height=350, showlegend=False)
                    ...
                    <div style="display: flex; flex-direction: column; gap: 15px; margin-top: 20px;">
                        ...
                        <div style="display: flex; justify-content: space-between; align-items: flex-start;">
                            ...
                            <div style="text-align: right;">
                                <div style="font-weight: 700; color: #ef4444;">{max_row['price_억']:.1f}억</div>
                                <div style="font-size: 0.8rem; color: #64748b;">{max_row['BLDG_NM']} ({max_row['ARCH_AREA']:.1f}㎡)</div>
                            </div>
                        </div>
                        <div style="display: flex; justify-content: space-between;">
...
def render_footer():
    """하단 푸터 렌더링 (Codexstyle 스타일 반영)"""
    footer_html = """
<div class="main-footer">
<div class="footer-content">
<div class="footer-header">
해당 서비스는 <b>패스트캠퍼스 이너서클 데이터분석 과정</b> 프로젝트를 통해 제작되었습니다.
</div>
<div class="footer-info-row">
<span>팀: fcicb6 데이터를 9해조</span>
<span class="footer-separator">|</span>
<span>프로젝트명 : 서울 슬세권 분석 시스템</span>
<span class="footer-separator">|</span>
<span>제작일 : 2026. 02</span>
</div>
<div class="footer-info-row">
<span>활용 데이터 : 서울시 소상공인, 교통(버스/지하철), 의료, 금융, 유통, 교육, 공원, 경찰시설, 학교, 스타벅스 등</span>
</div>
<div class="footer-info-row">
<span>Tel. 010-XXXX-XXXX</span>
<span class="footer-separator">|</span>
<span>E-mail. <a href="mailto:sample@sample.co.kr" style="color: inherit; text-decoration: none;">sample@sample.co.kr</a></span>
</div>
<div class="footer-link-main">
SEOUL SEULSEKWON ANALYTICS
</div>
